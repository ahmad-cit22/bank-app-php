...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($errorBag->hasErrors()) {
    ...
    if ($amount > $balance) {
      Message::flash('error', 'Insufficient balance to withdraw!');
      header('Location: ./withdraw.php');
      exit;
    }

    try {
      $transaction->save($user['email'], $amount, 'withdraw');
      Message::flash('success', 'Amount withdrawn successfully.');
      header('Location: ./withdraw.php');
      exit;
    } catch (Exception $e) {
            ...
            <div class="flex items-center px-2 lg:px-0">
              <div class="hidden sm:block">
                <div class="flex space-x-4">
                  ...
                  <a href="./customer_dashboard.php" class="hover:bg-emerald-500 hover:bg-opacity-75 text-white rounded-md py-2 px-3 text-sm font-medium" aria-current="page">Dashboard</a>
                  ...
                  <a href="./withdraw.php" class="text-white bg-emerald-700 hover:bg-opacity-75 rounded-md py-2 px-3 text-sm font-medium">Withdraw</a>
                ...
                <div>
                  <button @click="open = !open" type="button" class="flex rounded-full bg-white text-sm focus:outline-none" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                    ...
                    <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100">
                      <span class="font-medium leading-none text-emerald-700"><?= $user['name_initial'] ?></span>
                    </span>
                  </button>
                </div>
